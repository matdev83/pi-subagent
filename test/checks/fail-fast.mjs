...
import { interruptRun } from "../../src/orchestrate/interrupt.ts";
...
try {
	...
	// Terminate any durable workers that were actually spawned before removing
	// the temp tree: on Windows a running worker holds the directory (its cwd)
	// and rmdir fails with EBUSY until the worker exits.
	if (
		partialError !== undefined &&
		Array.isArray(partialError.startedRunIds) &&
		partialError.startedRunIds.length > 0
	) {
		for (const runId of partialError.startedRunIds) {
			await interruptRun({
				cwd: asyncCwd,
				runId,
				signal: "SIGKILL",
	...
	if (cwd !== undefined) await rmTree(cwd);
}
